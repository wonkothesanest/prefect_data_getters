...
class TestPhase4Elasticsearch:
    ...
    def test_document_save_method(self):
        ...
        with patch('prefect_data_getters.stores.documents_new.UnifiedDocumentStore') as mock_store_class:
            ...
            assert result == True
            mock_store.store_documents.assert_called_once_with([doc], "test_store")
    
    def test_document_delete_method(self):
        """Test AIDocument delete method"""
        with patch('prefect_data_getters.stores.documents_new.UnifiedDocumentStore') as mock_store_class:
            mock_store = Mock()
            ...
            assert result == True
            mock_store.delete_document.assert_called_once_with("test-123", "test_store")
    
    def test_document_search_method(self):
        """Test AIDocument search class method"""
        with patch('prefect_data_getters.stores.documents_new.UnifiedDocumentStore') as mock_store_class:
            mock_store = Mock()
            mock_results = [AIDocument("result 1"), AIDocument("result 2")]
            mock_store.search_documents.return_value = mock_results
            mock_store_class.return_value = mock_store
            
            results = AIDocument.search("test query", "test_store", top_k=5)
            
            assert len(results) == 2
            assert results[0].page_content == "result 1"
            mock_store.search_documents.assert_called_once_with("test query", ["test_store"], top_k=5)
    
    def test_backward_compatibility_upsert_documents(self):
        """Test backward compatibility function"""
        with patch('prefect_data_getters.stores.elasticsearch_compatibility.get_unified_store') as mock_get_store:
            mock_store = Mock()
            mock_store.store_documents.return_value = {'elasticsearch': {'success': 2, 'failed': 0}}
            ...
            mock_store.store_documents.assert_called_once()
            call_args = mock_store.store_documents.call_args
